...
class EmbeddingStore:
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        ...
        return {
            "id": doc.id if doc.id else f"id_{self._next_index}",
            "content": doc.content,
            "metadata": metadata,
    ...
    def _search_records(self, query: str, records: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        ...
        scored_records = []
        for rec in records:
            similarity = _dot(query_emb, rec["embedding"])
            scored_records.append({**rec, "score": similarity})
    ...
    def search_with_filter(self, query: str, top_k: int = 3, metadata_filter: dict = None) -> list[dict]:
        ...
        if self._use_chroma:
            ...
            # Format results... (omitted for brevity, similar to search)
            formatted = []
            for i in range(len(results["ids"][0])):
                formatted.append({
                    "id": results["ids"][0][i],
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        """
        if self._use_chroma:
            # For Chroma, we assume metadata has 'doc_id'
